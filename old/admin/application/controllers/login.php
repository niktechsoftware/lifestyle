...
class Login extends CI_Controller{
	...
	public function index(){
		...
		$this->db->where("client_code",$this->uri->segment(3));
		$val = $this->db->get("general_settings");
		if($val->num_rows()>0){
		    $val1=$val->row();
		$data['title'] = $val1->your_school_name;
		$this->load->view("admin/login",$data);
		}else{
		    echo "Bad Request Please Contact Hosting Provider";
		}
	}
	...
	function logout()
	{
		$this->session->unset_userdata();
		$this->session->sess_destroy();
		redirect(base_url().'login/index/80');
	}
